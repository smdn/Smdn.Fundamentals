...
public class MimeEncodingTests {
  ...
  public void Decode_ContainsLanguageSpecification()
  {
    Assert.That(
      ...
      Is.EqualTo("Keith Moore")
    );

    Assert.That(encoding, Is.EqualTo(MimeEncodingMethod.QEncoding));
    Assert.That(charset, Is.EqualTo(Encoding.ASCII));
  }
  ...
  public void Decode_QEncoding(string input, string expected)
    => Assert.That(MimeEncoding.Decode(input), Is.EqualTo(expected));
  ...
  public void Encode_QEncoding_Whitespaces(string input, string charset, string expected)
    => Assert.That(MimeEncoding.Encode(input, MimeEncodingMethod.QEncoding, Encoding.GetEncoding(charset)), Is.EqualTo(expected));
  ...
  public void Decode_QEncoding_Charsets(string input, string expectedResult, string expectedCharset)
  {
    ...
    Assert.That(encoding, Is.EqualTo(MimeEncodingMethod.QuotedPrintable), nameof(encoding));
    Assert.That(charset, Is.EqualTo(Encoding.GetEncoding(expectedCharset)), nameof(charset));
  }
  ...
  public void Decode_BEncoding(string input, string expectedResult, string expectedCharset)
  {
    ...
    Assert.That(encoding, Is.EqualTo(MimeEncodingMethod.Base64), "utf8");
    Assert.That(charset, Is.EqualTo(Encoding.GetEncoding(expectedCharset)), "utf8");
  }
  ...
  public void Decode_InvalidEncoding_ConvertToAlternativeText()
  {
    ...
    var ret = MimeEncoding.Decode(
      ...
      (Encoding c, string m, string t) => {
        called = true;
        Assert.That(c, Is.EqualTo(Encoding.UTF8));
        Assert.That(m, Is.EqualTo("x"));
        Assert.That(t, Is.EqualTo("baz"));
        return "baz";
      }
    );

    Assert.That(called, Is.True, "called");
    Assert.That(ret, Is.EqualTo("foobazbar"));
    ...
    Assert.That(
      ...
      Is.EqualTo("<alternative text>")
    );
    Assert.That(charset, Is.EqualTo(Encoding.UTF8));
    Assert.That(encoding, Is.EqualTo(MimeEncodingMethod.None));
  }
  ...
  public void Decode_CharsetNotBeSupported()
  {
    ...
    Assert.That(ex!.EncodingName, Is.EqualTo("invalid"), "EncodingName");
  }
  ...
  public void Decode_ValidCharset_SelectFallback()
  {
    ...
    Assert.That(ret, Is.EqualTo("漢字abcかな123カナ"));
    Assert.That(encoding, Is.EqualTo(MimeEncodingMethod.QEncoding));
    Assert.That(charset, Is.EqualTo(Encoding.UTF8));
  }
  ...
  public void Decode_InvalidCharset_SelectFallback_ReturnNull()
  {
    ...
    Assert.That(ex!.EncodingName, Is.EqualTo("x-invalid"), "EncodingName");
    Assert.That(called, Is.True);
    Assert.That(encoding, Is.EqualTo(MimeEncodingMethod.None));
